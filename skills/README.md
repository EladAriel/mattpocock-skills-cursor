...
2. Link skills into Cursor (global — available in all projects):

```bash
./scripts/link-skills-cursor.sh
```

   For a single project instead, run from that project's root:

```bash
/path/to/mattpocock-skills-cursor/skills/scripts/link-skills-cursor.sh --project
```

3. Reload Cursor (`Cmd/Ctrl+Shift+P` → **Developer: Reload Window**).

4. Invoke **setup-matt-pocock-skills** in your project (attach via `@setup-matt-pocock-skills` or mention it in chat). It will:
...
These split on one axis — who can invoke them. **User-invoked** skills are reachable only when you explicitly invoke them (e.g. `@grill-me` in Cursor); their job is to orchestrate. **Model-invoked** skills can be invoked by you _or_ reached for automatically by the agent when the task fits; they hold the reusable discipline. A user-invoked skill may invoke model-invoked skills, but never another user-invoked one.
