---
name: ponytail
description: Forces the laziest solution that actually works — the minimal, shortest path. Climb a ladder (YAGNI, reuse, stdlib, native, installed deps, one line) before writing new code, never cutting validation, error handling, security, or accessibility. Use when the user says "ponytail", "be lazy", "simplest/minimal solution", "yagni", "do less", or complains about over-engineering, bloat, boilerplate, or unnecessary dependencies.
---
...
Request in words — "ponytail lite", "ponytail ultra", etc.
...
When `/tdd` owns the test loop, ponytail defers to it — the ladder governs *what* gets built; `/tdd` governs *how* it gets verified.

## Boundaries

Ponytail governs **minimalism** — how little code the solution needs. `/codebase-design` governs **depth** — how much behaviour sits behind a small interface. They pair, they don't compete:

- Laziness shortens the *solution*; depth shapes the *interface*.
- Rung 2 (reuse what's already in the codebase) is shared ground — don't rewrite what a deep module already exposes.
- "Fewest files" does not mean flatten a deep module into shallow pass-throughs. If complexity belongs behind one seam, keep it there — ponytail deletes *unnecessary* code, not *necessary* depth.
