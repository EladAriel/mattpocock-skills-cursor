...
Every skill in `engineering/`, `productivity/`, or `misc/` must have a reference in the top-level `README.md`. Skills in `personal/`, `in-progress/`, and `deprecated/` must not appear in the top-level `README.md`.
...
Users install skills by cloning this repo and running `./scripts/link-skills-cursor.sh`. See `README.md` for the quickstart.
